...
- `agent_trader/` — the honest agent desk: `execution` (refusing chokepoint: caps, kill switch, idempotent fills, implied-zero exits, stale-mark flags), `mandate` (V4: deterministic pass/warn/BLOCK on every proposal — concentration on invested capital, sells always pass, missing data skips loudly; a BLOCK stops the cycle before execution), `loop` (one unattended cycle; LLM proposes, deterministic strategy decides), `team`, `brief`, fleet/gate/desk modules.
